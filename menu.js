fetch('menu.json')
//Que hace? Esta linea inicia la carga del archivo menu.json fetch es una funcion  de JavaScript que facilita la realizacion de solicitudes de red, como obetner datos de un archivo o un endpoint de API,
//Como funciona? fetch devuelve una promesa que, cuando se resuelve, te da acceso a la respuesta de la solicitud. Esta respuesta es directamente los datos en formato JSON, sino un objeto de respuesta que incluye varios detalles sobre la respuesta misma.
...
then(data=>{
    ...
    ('menu-container');
    //Que hace? Aqui se procesan los datos JSON ya convertidos.Se obtiene uuna referencia al contenidos del menu en el HTML mediante getElementById('menu-container'), y luego se itera sobre los elementos (categorias) del menu.
    data.items.forEach(category => {
        ...
        let tableBody = '';
        //Como funciona? Para cada  categoria en los datos, se realizan varios pasos: Crear un titulo para la categoria (se crea un elemento <h2> para el titulo de la categoria (category.category), luego se agrega al contenedor del menu).
        ...
        category.items.forEach (item =>{
            tableBody +=`<tr>
            <td><imag src=${item.image} alt="${item.name}"</td>
